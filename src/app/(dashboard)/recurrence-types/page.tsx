...
export default function RecurrenceTypesPage() {
    ...
    const [recurrenceTypes, setRecurrenceTypes] = React.useState<RecurrenceType[]>([]);
    ...
    React.useEffect(() => {
        if (!isSessionLoading && !session?.user) {
            router.replace(`/login?callbackUrl=${encodeURIComponent("/recurrence-types")}`);
        }
    ...
    const fetchRecurrenceTypes = React.useCallback(async () => {
        ...
        try {
            ...
            toast.error(
                error instanceof Error ? error.message : "Failed to fetch recurrence types",
            );
            setRecurrenceTypes([]);
        } finally {
            setIsLoading(false);
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="p-4 sm:p-6 space-y-4">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <CreateRecurrenceTypeDialog onRecurrenceTypeCreated={fetchRecurrenceTypes} />
                </div>
                    ...
                    <Card>
                        <CardContent className="p-4">
                            ...
                            <div className="text-2xl font-black text-green-600">
                                {isLoading ? "-" : recurrenceTypes.filter((r) => r.is_active).length}
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
